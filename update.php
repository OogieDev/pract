...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if (!$errors){
        ...
        $sql = $db->prepare("UPDATE students SET name = ?, surname = ?, father_name = ?, sex = ?, birth_date = ?, birth_city = ?, birth_region = ?, birth_country = ?, cityzen = ?, addr = ?, house_index = ?, phone = ?, email = ?, document_type = ?, document_seria = ?, document_number = ?, document_date = ?, document_giver = ?, faculty = ?, speciality = ?, curs = ?, study_form = ? WHERE id = ?");
        $sql->execute(array($_POST['name'], $_POST['surname'], $_POST['father_name'], $sex, $_POST['birth_date'], $_POST['birth_city'], $_POST['birth_region'], $_POST['birth_country'], $_POST['cityzen'], $_POST['addr'], $_POST['house_index'], $_POST['phone'], $_POST['email'], $_POST['document_type'], $_POST['document_seria'], $_POST['document_number'], $_POST['document_date'], $_POST['document_giver'], $_POST['faculty'], $_POST['speciality'], $_POST['curs'], $_POST['study_form'], $_POST['item_id']));
...
elseif ($_SERVER['REQUEST_METHOD'] == "GET") {
    $query = $db->query("SELECT * FROM students WHERE id = " . $_GET['id']);
    $item = "";
    if (!$item = $query->fetch()){
        header("Location: http://www2.cs.vsu.ru/~glushhenko_v_v/");
        die();
